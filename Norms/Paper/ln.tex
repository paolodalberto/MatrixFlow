...
logical division of the computation but we want to wraw attention to
...
\ref{fig:aie}: L3 = DDR = infinity, L2 is composed of four memtile of
512KB, and each memtile has a connection to one column of cores and
one different connection to a row. For this paper, we assume that the
connections are broadcasts: the same data is broadcast to all cores and
the core will select a subpart. Each Core has eight banks of 8KB each
composing the lowest level L1, we give two banks for inputs for ping
pong, two for weights, two for outputs, and two banks for Temporary
Space.

Intuitively, Tiling is a {\em spatial} and a {\em temporal}
partition. Where, partition describe a way to take a matrix and split
into parts. 

\begin{definition}
  We assume a layout: row major.  Consider a matrix $\Vc{X} \in
  \R^{M\times N}$, a row partition is easily specified by a offset
  address.
  \begin{equation}
    [\Vc{X} ]_{r=4} = \
    \begin{pmatrix}
    ...
    \end{pmatrix}\\
  \end{equation}
  then Address($\Vc{X}_i$) = $(i-1)*\frac{M}{4}*N$ and we could easily
  represent this as a $\dot\sum_i^r \Vc{X}^i$.
  \begin{equation}
    [\Vc{X} ]_{c=4} = \
    \begin{pmatrix}
    ...
    \end{pmatrix}
  \end{equation}
  then Address($\Vc{X}_i$) = $(i-1)*\frac{N}{4}$ and we could easily
  $\dot\sum_i^c \Vc{X}^i$.  This represent a {\bf spatial partition}, each
  partition has a buffer $\Vc{X}^i$ and each is independent.
\end{definition}
...
\begin{definition}
  ...
  $\Vc{X}^k_{i,j}$ is dump into in order so that the whole buffer
  eventually has been touched/transfer ($\sum_{i}^c\Vc{X}^k_{i}$ by
  column and $\sum_{i}^r\Vc{X}^k_{i}$ by row).
\end{definition}
...
\begin{definition}
  ...
  partition $\sum_{i}^r\Vc{X}^k_{i}$ with tile $\Vc{X}^k_{i}$ with
  further split by row. ($\sum_{i}^c\Vc{X}^k_{i}$ by column or both
  $\sum_{i,j}^{rc}\Vc{X}^k_{i,j}$).
\end{definition}
...
\begin{definition}
  ...
  $\Vc{X}$ is the original buffer, there are four spatial tiles
  $\Vc{X}^k$, each is the buffer that will be streamed by temporal
  partition $\sum_{i}^r\Vc{X}^k_{i}$ with tile $\Vc{X}^k_{i}$ (by row and by row)
\end{definition}
...
In general a complete tiling for L3, L2, and L1 could be summarized by
\begin{equation}
...
Because of the nature of the computation and its associativity of the
projection, when any spatial or temporal partition is by column, then
all the following are by column.

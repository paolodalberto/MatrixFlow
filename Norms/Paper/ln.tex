...
the sense that we use the memory hierarchy to stream rows and
...
Let us rewrite the blocked algorithm here
...
We want to focus on the loops for the Reduction and Normalization. The
loops read the same input in the same order and with the same
partition. We highlight this by introducing a repetition loop. 
